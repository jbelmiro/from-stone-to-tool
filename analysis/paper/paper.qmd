...
solu_stats <- solutrean_dataset %>% 
  ...
  mutate(CLASS = case_when(
    ...
    TRUE ~ CLASS)) %>%
  filter(CLASS %in% c("Flake", "ElongatedProd", "ThinningFlake", "Core", "BlankFragProx", "RetouchedPiece")) %>%
...
proto_stats_blanks <- phase_dataset %>% 
  ...
  mutate(CLASS = case_when(
    ...
    TRUE ~ CLASS)) %>% 
  filter(CLASS %in% c("Flake", "ElongatedProd", "Core", "BlankFragProx", "RetouchedPiece")) %>%
  select(RMTYPE, CLASS, THICKNESS, MAXWIDTH, LENGTH, WEIGHT) %>% 
  group_by(RMTYPE, CLASS) %>% 
  summarise(across(c(THICKNESS, MAXWIDTH, LENGTH, WEIGHT), list(mean = mean, sd = sd, max = max, min = min)))

# Gravettian statistics
grav_stats_blanks <- phase_dataset %>%
