...
solutrean_stats_blank <- solutrean_stats |>  
  ...
  group_by(RMTYPE, CLASS) |> 
  summarise(across(c(MAXWIDTH, LENGTH, WEIGHT), list(mean = mean, sd = sd, max = max, min = min)))
# For cores
...
kable(head(solutrean_stats_blank))
...
kable(head(gravettian_stats_blank))
...
kable(head(p.value_blanks))
kable(head(p.value_cores))
